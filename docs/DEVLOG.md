...
### 第三輪精緻化 + Google Search Console
- 已設定 Google Search Console（HTML 標記驗證 meta 加入 index.html、提交 sitemap）。
- 選中吸菸點：地圖標記改為**紅色**並放大（原本僅些微放大不夠顯眼）。
- 「以 Google Maps 導航」連結加上 `travelmode=walking`，預設走路。
- 新增**聯繫開發者**頁（第 4 分頁）：自由留言 + 選填聯絡方式 → 寫入 Supabase `feedback` 表（RLS 僅允許新增、不可讀，開發者由後台檢視）；前端每小時最多 3 則防灌水。
- 單元 14/14、E2E 12/12（含走路模式、聯繫頁送出）。
